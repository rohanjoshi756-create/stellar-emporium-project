...
export function CategoryGrid({ title = "Shop by Category" }: { title?: string }) {
  return (
    <>
      ...
      <section className="cv-auto pt-6 pb-10 sm:pt-8 sm:pb-14">
        ...
        <div className="container-x flex gap-2.5 sm:gap-3.5 overflow-x-auto no-scrollbar snap-x snap-mandatory pb-2 edge-fade">
          {categories.map((c) => (
            <Link
              key={c.handle}
              to="/collections/$slug"
            ...
            >
              ...
              <div className="px-1.5 py-2 text-[11px] sm:text-[12px] font-semibold text-center leading-tight text-foreground group-hover:text-primary transition-colors truncate whitespace-nowrap">
                {c.title}
              </div>
            </Link>
          ))}
